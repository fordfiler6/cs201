...
public class GameBoard extends JPanel
{
	...
	public static final int SPACE_WIDTH = GameDisplay.WIDTH/((NUM_SPACES/4)+1);
	public static final int SPACE_HEIGHT = GameDisplay.HEIGHT/((NUM_SPACES/4)+1);
	...
	private void takeTurn()
	{
		...
		while(input == null)
		{
			String[] choices = { "Roll", "Buy Upgrades", "1", "2","3"};
		    input = (String) JOptionPane.showInputDialog(null, "What would you like to do?",
		        "Player "+(curPlayer+1), JOptionPane.QUESTION_MESSAGE, null, // Use
		                                                                        // default
		                                                                        // icon
		...
		else if(input == "Buy Upgrades")
		{
			...
			{
				options[i] = choices.get(i).getName() + ". Cost: $"+choices.get(i).getUpgradeCost();
			}
		    input = (String) JOptionPane.showInputDialog(null, "What would you like to do?",
		        "Player "+(curPlayer+1), JOptionPane.QUESTION_MESSAGE, null, // Use
		                                                                        // default
	...
	private int rollDice()
	{
		...
		if(!sim)
			JOptionPane.showMessageDialog(null, "You rolled a "+die1+ " and a "+die2+". Move "+(die1+die2) +" spaces.");
		return die1+die2;
	}
	...
	{	
		BufferedImage image = new BufferedImage(GameDisplay.WIDTH, GameDisplay.HEIGHT, BufferedImage.TYPE_INT_ARGB);
	...
	public void paintPlayers(Graphics g)
	{
		for(int i=0;i<players.length;i++)
		{
			...
			BufferedImage toDraw = scale(p.getPiece().getImage(),BufferedImage.TYPE_INT_ARGB, 25, 25, .5, .5);
	...
	public void paintPlayerInfo(Graphics g)
	{
		...
		for(int i=0;i<players.length;i++)
		{
			...
			g.drawString("Player: "+(i+1), x+p.getPiece().getIconWidth()+PADDING, y+LINE_HEIGHT);
			g.drawString("Balance: $"+p.getBalance(), x+p.getPiece().getIconWidth()+PADDING, y+LINE_HEIGHT*2);
			ArrayList<Property> properties = p.getOwnedProperties();
			g.drawString("Properties: ", x, y+p.getPiece().getIconHeight()+LINE_HEIGHT);
			for(int k=0;k<properties.size();k++)
			{
				String prop = properties.get(k).getName() + ". Rent: $" +properties.get(k).calculateRent() +" ("+properties.get(k).getUpgradeString()+")";
				g.drawString(prop, x, y+p.getPiece().getIconHeight()+LINE_HEIGHT*(k+2));
			}
			if(i%2==0)
			{
				x = x*3;
	...
	public void paintBoard(Graphics g)
	{
		...
		for(int i =1;i<=NUM_SPACES/4;i++)
		{
			...
			if(i==NUM_SPACES/4)
				width = width-3;
			g.drawLine(width, GameDisplay.HEIGHT_BOARD-SPACE_HEIGHT, width, GameDisplay.HEIGHT_BOARD+100);
			
			int spaceNum = i;
			boardSpaces.get(spaceNum).setBoardLocation(++spaceCount);
			
			display = "<html><body style='width:"+(SPACE_WIDTH-30)+" px; padding: 5px; text-align:center;'>"
		                + boardSpaces.get(spaceNum).getName();
			if(boardSpaces.get(spaceNum) instanceof Property)
			{
				display += "<br>$"+((Property)boardSpaces.get(spaceNum)).getCost();
			}
            ...
            textLabel.paint(g2);
            g.drawImage(bi, GameDisplay.WIDTH-SPACE_WIDTH*(i+1)-(((NUM_SPACES/4)-i+1)*2), GameDisplay.HEIGHT_BOARD-SPACE_HEIGHT, null);
            boardSpaces.get(spaceNum).setX(GameDisplay.WIDTH-SPACE_WIDTH*(i+1)-(((NUM_SPACES/4)-i+1)*2));
			boardSpaces.get(spaceNum).setY(GameDisplay.HEIGHT_BOARD-SPACE_HEIGHT);
		...
		for(int i =1;i<=NUM_SPACES/4;i++)
		{
	        ...
	        textLabel.paint(g2);
	        g.drawImage(bi,0, GameDisplay.HEIGHT_BOARD-SPACE_HEIGHT*(i+1)+((i-1)*8), null);
			boardSpaces.get(spaceNum).setX(0);
			boardSpaces.get(spaceNum).setY(GameDisplay.HEIGHT_BOARD-SPACE_HEIGHT*(i+1)+((i-1)*8));
		}
		...
		for(int i =0;i<=NUM_SPACES/4;i++)
		{
			...
			if(i!= NUM_SPACES/4)
			{
		        ...
		        textLabel.paint(g2);
		        g.drawImage(bi, GameDisplay.WIDTH-SPACE_WIDTH*(i+1)-(((NUM_SPACES/4)-i+1)*2), 0, null);
				boardSpaces.get(spaceNum).setX(GameDisplay.WIDTH-SPACE_WIDTH*(i+1)-(((NUM_SPACES/4)-i+1)*2));
		...
		for(int i =1;i<=NUM_SPACES/4;i++)
		{
			...
			if(i==NUM_SPACES/4)
				width = width+4;
			g.drawLine(GameDisplay.WIDTH_BOARD-SPACE_WIDTH, width, GameDisplay.WIDTH_BOARD+100, width);
			if(i!= NUM_SPACES/4)
			{
		        ...
		        textLabel.paint(g2);
		        g.drawImage(bi, GameDisplay.WIDTH_BOARD-SPACE_WIDTH, GameDisplay.HEIGHT_BOARD-SPACE_HEIGHT*(i+1)+((i-1)*8), null);
				boardSpaces.get(spaceNum).setX(GameDisplay.WIDTH_BOARD-SPACE_WIDTH);
				boardSpaces.get(spaceNum).setY(GameDisplay.HEIGHT_BOARD-SPACE_HEIGHT*(i+1)+((i-1)*8));
			}
		}
	}
	//FROM STACKOVERFLOW
	public static BufferedImage scale(Image in, int imageType, int dWidth, int dHeight, double fWidth, double fHeight) 
	{
